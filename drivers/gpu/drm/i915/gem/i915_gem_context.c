...
static bool __cancel_engine(struct intel_engine_cs *engine)
{
	 ...
	 * instead.
	 */
	if (IS_ACTIVE(CPTCFG_DRM_I915_PREEMPT_TIMEOUT) &&
...
set_engines__load_balance(struct i915_user_extension __user *base, void *data)
{
	...
	if (intel_uc_uses_guc_submission(&set->ctx->i915->gt.uc))
...
{
	unsigned int ban_score;

	if (file_priv->dev_priv->params.score_banned > I915_CLIENT_SCORE_BANNED)
		ban_score = file_priv->dev_priv->params.score_banned;
	else
		ban_score = I915_CLIENT_SCORE_BANNED;

	return atomic_read(&file_priv->ban_score) >= ban_score;
}
